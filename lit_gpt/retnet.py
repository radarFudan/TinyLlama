...
from lit_gpt.rmsnorm import FusedRMSNorm as RMSNorm
...
class RetNetRelPos(nn.Module):
    ...
    def reset_parameters(self) -> None:
        # print("reset_parameters called in retnet, consider implement it and check the usage")
        pass
...
class RetNetDecoder(nn.Module):
    ...
    def forward(
        ...
        **kwargs
        ...
        # embed tokens
        # mid_x, _ = self.forward_embedding(
        #     x, token_embeddings, incremental_state
        # )
        # print("x shape")
        x_size = x.size(1)
        mid_x = x
        ...
        if self.chunkwise_recurrent and x_size % self.recurrent_chunk_size != 0:
            padding_len = self.recurrent_chunk_size - x_size % self.recurrent_chunk_size
            slen = x_size + padding_len
            mid_x = F.pad(mid_x, (0, 0, 0, padding_len))
        else:
            slen = x_size
        # relative position
        ...
        for idx, layer in enumerate(self.layers):
            ...
            inner_states.append(mid_x)
            
        if self.chunkwise_recurrent and x_size % self.recurrent_chunk_size != 0:
            mid_x = mid_x[:, :x_size, :]

        if self.layer_norm is not None:
            mid_x = self.layer_norm(mid_x)

        # print("features_only is", features_only)
        if not features_only:
            mid_x = self.output_layer(mid_x)

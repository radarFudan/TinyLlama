...
class CausalSelfAttentionMamba(nn.Module):
    def __init__(self, config: Config) -> None:
        ...
        self.mamba = Mamba(
            ...
            d_model=3 * config.head_size, # Model dimension d_model
            # d_state=16,  # SSM state expansion factor
        ...
        # self.out_proj = nn.Linear(3 * config.head_size, 1 * config.head_size, bias=config.bias)
    ...
    def scaled_dot_product_attention(
        ...
        bsz, nheads, seqlen, headdim = qkv.shape

        y = einops.rearrange(qkv, 'b nheads seqlen headdim -> (b nheads) seqlen (headdim)') # y (batch_size * nheads, seqlen, headdim)
        y = self.mamba(y) # y (batch_size, nheads, seqlen, 3 * headdim)
        y = einops.rearrange(y, '(b nheads) seqlen (headdim) -> b nheads seqlen headdim', b=bsz, nheads=nheads, headdim=headdim) # y (batch_size, nheads, seqlen, 3 * headdim)
...
class CausalSelfAttentionSSM(nn.Module):
    def __init__(self, config: Config) -> None:
        super().__init__()
        shape = (config.n_head + 2 * config.n_query_groups) * config.head_size
        # key, query, value projections for all heads, but in a batch
...
class SSM_Hyena(nn.Module):
    ...
    No change of shape in this case, input output shape are all Batch * Heads * length * n_ssm
    """
    def __init__(self, config: Config) -> None:
        super().__init__()
        ...
        lambdas_weights = torch.rand(1, 1, 1, config.n_ssm) # From 0 to 1, B * H * T * D
    ...
    def forward(
        ...
        B, H, T, C = x.size()  # batch size, heads, sequence length, embedding dimensionality (n_embd)
        ...
        Lambda_elements = lambdas * torch.ones(1, 1, T, 1).to(self.lambdas.device) # B * H * T * C

        # TODO, Notice that axis cannot be -2
        y = associative_scan(nested_func, (Lambda_elements[:,:,:,:], x), axis=2)[1] + self.D(x) # B * H * T * C

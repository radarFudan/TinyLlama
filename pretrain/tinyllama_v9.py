...
wandb_logger = WandbLogger(name=f"{model_name}_v{version}", id=f"{model_name}_v{version}", project="TL_final")
...
def setup(
    ...
    main(fabric, train_data_dir, val_data_dir, test_data_dir, resume)


def main(fabric, train_data_dir, val_data_dir, test_data_dir, resume):
    ...
    if resume is True:
        ...
        resume = sorted(out_dir.glob(f"*-v{version}.pth"))[-1]
    if resume :
        fabric.print(f"Resuming training from {resume}")
        fabric.load(resume, state)

    train_time = time.perf_counter()
    train(fabric, state, train_dataloader, val_dataloader, test_dataloader, monitor, resume)
    fabric.print(f"Training time: {(time.perf_counter()-train_time):.2f}s")
    if fabric.device.type == "cuda":
        fabric.print(f"Memory used: {torch.cuda.max_memory_allocated() / 1e9:.02f} GB")


def train(fabric, state, train_dataloader, val_dataloader, test_dataloader, monitor, resume):
    model = state["model"]
    ...
    if test_dataloader is not None:
        t0 = time.perf_counter()
        val_loss = validate(fabric, model, test_dataloader)  # sanity check
        t1 = time.perf_counter() - t0
        fabric.print(f"Sanity check, test loss {val_loss:.4f}, test time: {t1 * 1000:.2f}ms")
    ...
    for  train_data in train_dataloader:
        ...
        input_ids = train_data[:, 0 : -1].contiguous()
        ...
        if val_dataloader is not None and not is_accumulating and state["step_count"] % eval_step_interval == 0:
            ...
            fabric.barrier()
        if test_dataloader is not None and not is_accumulating and state["step_count"] % eval_step_interval == 0:
            
            t0 = time.perf_counter()
            test_loss = validate(fabric, model, test_dataloader)
            t1 = time.perf_counter() - t0
            monitor.eval_end(t1)
            fabric.print(f"step {state['iter_num']}: test loss {test_loss:.4f}, test time: {t1 * 1000:.2f}ms")
            fabric.log_dict({"metric/test_loss": test_loss.item(), "total_tokens": model.config.block_size * (state["iter_num"] + 1) * micro_batch_size * fabric.world_size}, state["step_count"])
            fabric.log_dict({"metric/test_ppl": math.exp(test_loss.item()), "total_tokens": model.config.block_size * (state["iter_num"] + 1) * micro_batch_size * fabric.world_size}, state["step_count"])
            fabric.barrier()
            
        if not is_accumulating and state["step_count"] % save_step_interval == 0:
            # checkpoint_path = out_dir / f"iter-{state['iter_num']:06d}-ckpt.pth"
            checkpoint_path = out_dir / f"iter-{state['iter_num']:06d}-ckpt-v{version}.pth"
...
def create_dataloader(
    batch_size: int, block_size: int, data_dir: Path, fabric, shuffle: bool = False, seed: int = 12345, split="train"
) -> DataLoader:
    datasets = []
